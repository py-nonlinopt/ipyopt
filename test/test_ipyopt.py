...
def problem(n: int, with_hess: bool = False, wrap_eval_h=lambda f: f, **kwargs):
    _e_x = e_x(n)
    ...
    def eval_h(_x, lagrange, obj_factor, out):
        out[()] = numpy.full((n,), 2.0 * (obj_factor + lagrange[0]))
        return out
...
class TestIPyOpt(unittest.TestCase):
    def test_simple_problem(self):
        ...
        for with_hess in (False, True):
            with self.subTest(with_hess=with_hess):
                ...
                p = problem(n, with_hess=with_hess, wrap_eval_h=eval_h_call_checker)
                x, obj, status = p.solve(
                    x0, mult_g=constraint_multipliers, mult_x_L=zl, mult_x_U=zu
                )
    ...
    def test_problem_scaling():
        ...
        numpy.testing.assert_array_almost_equal(x, 3.0 * e_x(n))
